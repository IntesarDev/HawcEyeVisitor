...
type DraftState = {
  ...
  // حقول قديمة محتملة من النسخة السابقة (للهجرة الذاتية)
  date?: string;
  start?: string | null;
  hours?: number;
};
...
const heal = (s: DraftState) => {
  ...
  // لو كانت هناك حقول قديمة مسطحة، انقلها للنوع الحالي ثم امسحها
  if (s.date !== undefined || s.start !== undefined || s.hours !== undefined) {
    ...
    delete s.date; delete s.start; delete s.hours;
  }

  // تأكد أن كل الأنواع موجودة
...
export const { setType, setDate, setStart, setHours, resetCurrent, resetAll } = slice.actions;
